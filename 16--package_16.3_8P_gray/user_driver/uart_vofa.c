...
void vofa_parse_packet(char *packet)
{
    ...
    else if (cmd2 == '4')   // 循迹Kp  P4=0.03
    {
        if (cmd1 == 'P')
        {
            Line_Kp = value;
            Serial_Printf("Set Line_Kp = %.3f\r\n", value);
        }
        if (cmd1 == 'B')//Basespeed
        {
            BaseSpeed = value;
            Serial_Printf("Set BaseSpeed= %.d\r\n", value);
        }
        if (cmd1 == 'T')//转弯角度
        {
            turn_angle_deg   = value;
            Serial_Printf("Set TURN_ANGLE_DEG = %.3f\r\n", value);
        }
        // else if (cmd1 == 'I')
        // {
        //     Angle_PID.Ki = value;
