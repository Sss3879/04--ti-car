...
volatile uint32_t SysTick_Count = 0;   // 1ms 累加
volatile uint8_t  MPU6050_UpdateFlag = 0;        // 20ms 到，通知主循环
...
int main(void)
{
    ...
    // DL_GPIO_disableOutput(GPIOA, DL_GPIO_PIN_0);  // 释放
    // delay_ms(10);

    // uint32_t raw = DL_GPIO_readPins(GPIOA, DL_GPIO_PIN_0);
    // Serial_Printf("SDA read=0x%08lX (应0x00000001)\r\n", raw);
    ...
    while(1)
    {
        ...
        if (MPU6050_UpdateFlag == 1)
		{
			MPU6050_UpdateFlag = 0;
			MPU6050_GetData(&Ax,&Ay,&Az,&Gx,&Gy,&Gz);
		}

        Serial_Printf("%d,%d,%d,%d,%d,%d",Ax,Ay,Az,Gx,Gy,Gz);
