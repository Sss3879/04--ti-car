...
void Get_Analog_value(unsigned short *result)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        delay_us(10);                   // 等待传感器切换地址
        Anolag = adc_getValue(40);      // 40次采样取平均
...
void No_Mcu_Ganv_Sensor_Task_Without_tick(No_MCU_Sensor *sensor)
{
    ...
    convertAnalogToDigital(sensor->Analog_value, sensor->Gray_white, sensor->Gray_black, &sensor->Digtal);  // 二值化
    normalizeAnalogValues(sensor->Analog_value, sensor->Normal_factor, sensor->Calibrated_black, sensor->Normal_value, sensor->bits);  // 归一化
}
...
static uint8_t Gray_LostFlag = 0;
int16_t GraySensor_GetLineError(No_MCU_Sensor *sensor)
{
    ...
    int32_t black_strength = 0;

    /*
     * 默认 0~7 号传感器从左到右排列：
     * 左边为负，右边为正。
     */
    int16_t weight[8] = {-350, -250, -150, -50, 50, 150, 250, 350};
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        {
            black_strength = 4096 - normal[i];
        }

        if (black_strength > 300)
        {
            sum += black_strength;
            weighted_sum += black_strength * weight[i];
        }
    }
    ...
    Gray_LastError = (int16_t)(weighted_sum / sum);
