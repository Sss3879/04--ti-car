...
static uint8_t icm42688_read_reg(uint8_t reg)
{
    ...
    SPI_CS(1);
#elif defined(ICM_USE_I2C)
	I2C_ReadReg(ICM42688_ADDRESS, reg, &regval, 1);
#endif
...
static void icm42688_read_regs(uint8_t reg, uint8_t* buf, uint16_t len)
{
    ...
    SPI_CS(1);
#elif defined(ICM_USE_I2C)
	I2C_ReadReg(ICM42688_ADDRESS, reg, buf, len);
#endif
}
...
static uint8_t icm42688_write_reg(uint8_t reg, uint8_t value)
{
    ...
    SPI_CS(1);
#elif defined(ICM_USE_I2C)
	I2C_WriteReg(ICM42688_ADDRESS, reg, &value, 1);
#endif
    return 0;
}
...
{
    uint8_t reg_val = 0xAA;
    bool read_ok;
    /* Allow the sensor to finish power-up before the first bus access. */
    ICM42688DelayMs(100);

    /* Probe communication before issuing any write or software reset.
     * Only one byte is requested. 0xAA remains unchanged if RX never
     * receives a byte, which distinguishes failure from a real 0x00. */
    read_ok = I2C_ReadReg(
        ICM42688_ADDRESS, ICM42688_WHO_AM_I, &reg_val, 1U);
		printf("ICM42688 read_ok=%u, WHO_AM_I=0x%02X, err=%u, MSR=0x%08lX\r\n",
        (unsigned int)read_ok, reg_val, (unsigned int)gI2cErrorCode,
        (unsigned long)gI2cErrorStatus);


    if(reg_val == ICM42688_ID)
    {
        /* Communication is proven; software reset is now safe. */
