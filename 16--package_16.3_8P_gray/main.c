...
float Line_Kp = 0.08f;
#define SPEED_MAX  320//280
#define SPEED_MIN -320

#define TURN_ANGLE_DEG        45.0f
#define TURN_SPEED            100//80
#define FIND_SPEED            40
#define LOST_CONFIRM_COUNT    2
#define FIND_CONFIRM_COUNT    2
#define LINE_TRACK_MIN_TIME_MS      3800u 
#define TIMER_0_PERIOD_MS           20u
#define LINE_TRACK_TIME_LIMIT       (LINE_TRACK_MIN_TIME_MS / TIMER_0_PERIOD_MS)
...
int16_t GraySensor_GetGPIOLineError(void)
{
    ...
    static const int16_t weight[8] = {-200, -140, -80, -30, 30, 80, 140, 200}; 
...
{
    /* 只在真正开始执行新的巡线状态时清零计时。 */
    if (RoadState != LastRoadState) {
        if ((RoadState == RECT_AB_LINE) ||
            (RoadState == RECT_BC_LINE) ||
            (RoadState == RECT_CD_LINE) ||
            (RoadState == RECT_DA_LINE)) {
            line_track_time = 0;
        }
        LastRoadState = RoadState;
    }

    switch (RoadState) {
        case RECT_AB_LINE:
            if ((line_track_time >= LINE_TRACK_TIME_LIMIT) && Rect_CornerDetected()) {
                StartAngle = ypr[0];
        ...
        case RECT_CD_LINE:
            if ((line_track_time >= LINE_TRACK_TIME_LIMIT) && Rect_CornerDetected()) {
                StartAngle = ypr[0];
                RoadState = RECT_D_TURN;
            }
            else { Car_LineControl(); }
            break;
        case RECT_D_TURN:
            Car_TurnRightControl();
            if (MyAbsFloat(MyAbsFloat(ypr[0]) - MyAbsFloat(StartAngle)) >= TURN_ANGLE_DEG) RoadState = RECT_D_FIND_LINE;
            break;
        ...
        if (Flag_20ms == 1)
        {
            ...
            if (++DisplayTick >= 3)
            {
                ...
                if (RunMode == MODE_RECT) {
                    ...
                    //      Corner_Trigger_Count,
                    //      (unsigned long)line_track_time);
                    BlueSerial_Printf("Time:%lu,Yaw:%-6.1f\r\n  ",(unsigned long)line_track_time,ypr[0] );
                }
            }
        }
...
void TIMER_0_INST_IRQHandler(void)
{
    switch (DL_TimerG_getPendingInterrupt(TIMER_0_INST))
    {
        case DL_TIMER_IIDX_ZERO:
            Flag_20ms = 1;
            if (line_track_time < LINE_TRACK_TIME_LIMIT) {
                line_track_time++;
            }
